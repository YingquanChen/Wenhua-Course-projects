...
% Find the first side peak in the autocorrelation
side_peak_threshold = 0.1; % Threshold for detecting peaks
side_peak_indices = find(r(2:end) < side_peak_threshold);
first_side_peak_lag = side_peak_indices(1); % Take the first side peak

% Calculate the period from the first side peak lag
period = first_side_peak_lag / samplingRate;

% Estimate frequency using the period
